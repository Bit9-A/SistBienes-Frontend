...
export default function Dashboard(props: { [x: string]: any }) {
  ...
  const isAdmin = useMemo(() => {
    ...
    return userProfile.tipo_usuario === 1 || userProfile.tipo_usuario === 5;
  }, [userProfile]);
      ...
      if (response?.token) {
        ...
        if (storedUser) {
          ...
          localStorage.setItem(
            'user',
            JSON.stringify({ ...userData, token: response.token }),
          );
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      } else if (timeLeft < 1200 && timeLeft > 600) {
        // Si quedan entre 10 y 20 minutos, intentar refrescar
  ...
  useEffect(() => {
    ...
    const fetchUserProfile = async () => {
      try {
        ...
        (axiosInstance.interceptors.request as any)._interceptorId =
          interceptorId;
      } catch (error) {
        console.error(
          'Error al obtener el perfil o configurar interceptor:',
          error,
        );
        if (isMounted) {
          navigate('/auth/sign-in');
    ...
    return () => {
      ...
      const interceptorId = (axiosInstance.interceptors.request as any)
        ._interceptorId;
  ...
  const getActiveNavbarText = useCallback(
