...
export interface GeneralConfig {
  ...
  url_logo: string;
  url_favicon: string;
  url_impresion_1: string | null; // Nuevo campo
  url_impresion_2: string | null; // Nuevo campo
...
export const createConceptoMovimientoIncorporacion = async (
  conceptoData: any,
) => {
  try {
...
export const updateConceptoMovimientoIncorporacion = async (
  id: number,
  conceptoData: any,
) => {
  try {
...
export const createConceptoMovimientoDesincorporacion = async (
  conceptoData: any,
) => {
  try {
    const response = await axiosInstance.post(
...
export const updateConceptoMovimientoDesincorporacion = async (
  id: number,
  conceptoData: any,
) => {
  try {
    const response = await axiosInstance.put(
      `/concept-desincorp/${id}`,
      conceptoData,
    );
    return response.data; // Devuelve el concepto actualizado
  } catch (error) {
